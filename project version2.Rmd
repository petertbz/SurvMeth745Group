...
# 3D Calcualte the PSU probability with composite MOS
psu_frame <- psu_frame %>%
  mutate(
...
ssu_frame <- ssu_frame %>%
  left_join(psu_frame %>% select(COMBINED_TRACT, PSU_MOS_composite, PSU_prob), 
            by = c("COMBINED_TRACT" = "COMBINED_TRACT"))
...
ssu_frame <- ssu_frame %>%
  ...
  mutate(
    SSU_MOS_composite = (MOS_1844 + MOS_4564 + MOS_65up) / PSU_MOS_composite
  ) %>%
  mutate(
